...
@section('mail_send')
    ...
    <form action="{{route('messge_send.index')}}" enctype="multipart/form-data" method="POST">
        @method("POST")
        @csrf
    <div class="mb-3">
        <label for="message_for_admin" class="form-label" style="color: white">Отправить собщение администратору</label>
        <textarea class="form-control" id="message" name="message" rows="3"></textarea>
        <label for="email_for_feedback" class="form-label mt-4" style="color: white">Укажите e-mail адрес для обратной связи.</label>
