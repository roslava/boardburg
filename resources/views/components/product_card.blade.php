            ...
            <div class="col-sm-6 col-md-6 col-lg-4 col-xl-3 mb-4" style="display: flex; flex-direction: column">
                <div class="card" style="height: 100%;">
                    <div class="row pt-3" style="justify-content: center; margin:0">
                        @if($skateFromBase->img)
                            <a style="width: 100%; display: flex; justify-content: center; "
                               href="{{ route('skates_base.show', $skateFromBase->id)}}"><img
                                    src="{{asset('/storage/uploads/'.$skateFromBase->img)}}" class="card-img-top"
                                    style="max-height: 200px; width: auto;" alt="{{$skateFromBase->name}}"
                                    title="{{$skateFromBase->name}}"></a>
                        @endif
                    </div>
                    <div class="card-body">
                        ...
                        <div style=" display: flex; justify-content: space-between; align-items: center">
                            ...
                            <div class="bb-product-card-price">{{$skateFromBase->price}} <span class="rub">c</span>
                            </div>
                        </div>
                    </div>
                    <div class="card_bottom m-3" style="display: flex; flex-direction: column; align-items: end">
                        @if (Auth::check())
                            <div class="card-technical rounded bg-light mt-0 mb-4 p-2 shadow"
                                ...
                                <div>Менеджер: {{$skateFromBase->user_id}}</div>
