class ShoppingCartBB {
    ...
    constructor(urlCardIndex, urlCardRender, urlAddToCard, urlCartUpdate, urlRemoveFromCard, urlCartIconsAdded, uploadsFolderUrl, csrf_token) {

        if (urlCartUpdate === undefined) urlCartUpdate = false;
        if (urlCartIconsAdded === undefined) urlCartIconsAdded = false;
        ...
        this.urlCartIconsAdded = urlCartIconsAdded;
                ...
                fetch(this.urlAddToCard + '?id=' + id)
                    ...
                    .then(() => {
                        ...
                        shoppingCartDeleteBtn.forEach(item => {
                            item.value = id
                            let shoppingCartDeleteBtn_ = document.querySelectorAll('.shopping-cart__delete-btn_');
                            shoppingCartDeleteBtn_.forEach(item => {
    ...
    removeProductByCartByModal() {
        ...
        const form = document.querySelector('.shopping-cart__delete-btn-form-modal');
        form.addEventListener('submit', function (e) {
            e.preventDefault();
            const formData = new FormData(this);
            // console.log(hideModalConfirm)
            ajaxSend(formData)
                .catch((err) => console.error(err))
    ...
    cartIconsAdded() {
        fetch(this.urlCartIconsAdded)
            .then(data => {
                return data.json()
            })
            .then(data => {
                let cartIcons = Array.from(document.getElementsByClassName('product-card__shopping-cart')); //all cart icons
                ...
                let addedCartIcons = isArray(data['cartItems']) //all cart icons added

                cartIcons.forEach(icon => {
                    icon.classList.remove('product-card__shopping_added')
                })
                cartIcons.forEach(icon => {
                    addedCartIcons.forEach(addedCartIcon => {
                        if (addedCartIcon == icon.dataset.id) {
                            icon.classList.add('product-card__shopping_added')
                        }
                    })
                })
            })
