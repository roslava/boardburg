...
let bbCatalogDropdown = document.querySelector('.catalog_back_drop')

bbNavCatalogBtn.forEach(item => {
    item.addEventListener('click', event => {
        item.classList.toggle('bb-catalog-btn_close')
        bbCatalog.forEach(item => {
            item.classList.toggle('bb-catalog_show')
        })
        dropdownToggler()
    })
})

bbCatalogDropdown.addEventListener('click', event => {
    bbCatalog.forEach(item => {
        item.classList.toggle('bb-catalog_show')
    })
    bbNavCatalogBtn.forEach(item => {
...
function dropdownToggler() {
    bbCatalogDropdown.classList.toggle('catalog_back_drop_show')
}
